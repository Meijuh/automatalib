 ...
 */
public final class FSM2DFAParser<I> extends FSMParser<I> {
     ...
     * @param inputParser the input parser (see {@link FSMParser#inputParser}).
        ...
        if (acceptIndex == -1 && acceptValue == -1) {
            ...
            if (dataVariableName.equals(acceptingDataVariableName)) {
                ...
                while (getStreamTokenizer().nextToken() == '"' && acceptValue == -1) {
                    ...
                    if (dataValue.equals(acceptingDataValue)) acceptValue = dataValueIndex;
                    else dataValueIndex++;
                }
    ...
    protected void parseStateVector() throws FSMParseException, IOException {
        ...
        for (int i = 0;
            ...
            if (i == acceptIndex) {
                try {
                    accepting = acceptValue == Integer.valueOf(value);
        ...
        } else states.put(getPartLineNumber(), accepting);
    }
    ...
    /**
     * Sets the initial state by checking which state has no incoming transition.
     *
     * @throws FSMParseException when an initial state could not be found, or when there is more than one initial state.
     */
    @Override
    protected void checkTransitions() throws FSMParseException {
        final Set<Integer> initialStates = new HashSet(states.keySet());
        for (Integer to: transitions.values()) initialStates.remove(to);
    ...
    private CompactDFA<I> parseDFA() throws FSMParseException, IOException {
        ...
        // add all the states
        for (Map.Entry<Integer, Boolean> state : states.entrySet()) dfa.addState(state.getValue());

        // set the initial state (substract one because the StreamTokenizer starts counting at 1
        dfa.setInitialState(getInitialState() - 1);

        // add all the transitions
        for (Map.Entry<Pair<Integer, I>, Integer> transition : transitions.entrySet()) {
            dfa.addTransition(transition.getKey().getFirst() - 1, transition.getKey().getSecond(), transition.getValue() - 1);
        }

        return dfa;
    }

    public static CompactDFA parse(Reader reader, Function inputParser, String acceptingDataVariableName, String acceptingDataValue) throws
    ...
    public static CompactDFA parse(File file, Function inputParser, String acceptingDataVariableName, String acceptingDataValue) throws
            IOException, FSMParseException {
        return parse(new FileReader(file), inputParser, acceptingDataVariableName, acceptingDataValue);
    }

    public static CompactDFA parse(String string, Function inputParser, String acceptingDataVariableName, String acceptingDataValue) throws
            IOException, FSMParseException {
        return parse(new StringReader(string), inputParser, acceptingDataVariableName, acceptingDataValue);
    }

    public static CompactDFA parse(InputStream inputStream, Function inputParser, String acceptingDataVariableName, String acceptingDataValue)
            throws IOException, FSMParseException {
        return parse(new InputStreamReader(inputStream), inputParser, acceptingDataVariableName, acceptingDataValue);
    }
}
