 ...
 *
 * This was actually quite horrible to implement.
 *
 * @see http://rers-challenge.org/2017/index.php?page=problemDescS#
 *
 ...
 */
public class Mealy2ETFWriterAlternating<S, I, T, O> extends ETFWriter<I, MealyMachine<S, I, T, O>> {
    ...
    protected void writeETF(MealyMachine<S, I, T, O> mealy, Alphabet<I> inputs) {
        ...
        for (S s : mealy.getStates()) oldStates.put(s, oldStates.size());

        // write the initial state, using the bi-map
        getPrintWriter().println("begin init");
        getPrintWriter().printf("%d%n", oldStates.get(mealy.getInitialState()));
                    ...
                    final Integer intermediateState = outputTransitions.computeIfAbsent(
                            outputTransition, ii -> {
                                ...
                                */
                                ...
                                getPrintWriter().printf("%d/%d %d%n", res, oldStates.get(n), outputIndex);
                                return res;
                            });
        ...
        for (int i = 0; i < oldStates.size(); i++) getPrintWriter().printf("\"%s\"%n", oldStates.inverse().get(i));
        for (int i = 0; i < outputTransitions.size(); i++) {
            getPrintWriter().printf("\"%s\"%n", outputTransitions.inverse().get(oldStates.size() + i));
        }
        ...
        for (int i = 0; i < outputIndices.size(); i++) {
            getPrintWriter().printf("\"%s\"%n", outputIndices.inverse().get(inputs.size() + i));
        }
        getPrintWriter().println("end sort");
    }

    public static void write(Writer writer, MealyMachine mealy, Alphabet inputs) {
        new Mealy2ETFWriterAlternating(writer).write(mealy, inputs);
    }
